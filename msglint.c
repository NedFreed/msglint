...
const static char version[] = "MIME Lint v1.1.0 2012-09-08";
...
#define RFC_MIME_TOKEN "RFC 2045 sec 5.1"
...
#define RFC_DSN        "RFC 3464"
#define RFC_MDN        "RFC 3798"
...
const static char date_syntax[] = DATE_SYNTAX;
const static char date_new_syntax[] = \
        "{O{Emon|tue|wed|thu|fri|sat|sun},W}{TD{OD}}W" \
        "{E:month|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec}W" \
...
#define GROUP_SADDR_SYNTAX "P:W{O" MBOX_SYNTAX \
       "{*,{Xonly one address allowed in group}W{*,W{W2}}" MBOX_SYNTAX "}};"
#define SADDRGP_SYNTAX MBOX_PRESYNTAX "|" GROUP_SADDR_SYNTAX "}C"
const static char ALIST_SYNTAX[] = \
        "{A" ADDRGP_SYNTAX "{*,W{*,W{W2}}" \
        ADDRGP_SYNTAX "}|{Xat least one address required}}";
const static char MLIST_SYNTAX[] = \
        "{A" MBOX_SYNTAX "{*,F0W{*,W{W2}}" \
        MBOX_SYNTAX "}|{Xat least one mailbox required}}";
const static char RECEIVED_SYNTAX[] = \
        "C{citem|M1|M2|O3|O4|O5|O4|O5|O6|S6|O7|O8|:{*{eA:item label|1:from:WD" \
        "{OW[{Xdomain-literal should be in comment}}|" \
        "2:by:WD|" \
        "3:via:W{E:'via' clause|UUCP}|" \
        "4:with:W{E:'with' item|SMTP|ESMTP|ESMTPS|ESMTPA|ESMTPSA|LMTP|LMTPS|LMTPA|LMTPSA}|" \
        "5:id:W{A<L@D>|AW|{Xinvalid 'id' item}}|" \
        "6:for:W{O<}LW{A@WD{O>}|{Xmissing domain in 'for' addr-spec}}|" \
        "7:state:W{T{Aauth{O/T{*T}}$|content{O/T{*T}}$|convert{O/T{*T}}$|moderation{O/T{*T}}$|normal{O/T{*T}}$|other{O/T{*T}}$|outbound{O/T{*T}}$|quarantine{O/T{*T}}$|timed{O/T{*T}}$|T{*T}{O/T{*T}}${Wunrecognized RFC 6729 state}|{Xinvalid RFC 6729 state}S}}|" \
	"8:priority:Wi|" \
        ":}C}}{s;}W" DATE_SYNTAX;
#define URL_SYNTAX \
        "<{Ehttp:://{T{R1*:{AA|D}}{R1*:.{R1*:{AA|D}}}}/{S>}|" \
        "https:://{T{R1*:{AA|D}}{R1*:.{R1*:{AA|D}}}}/{S>}|"   \
        "mailto::{O{AP<L@A>|L@A}{*,{AP<L@A>|L@A}}}{*?A=A}>|:" \
        "{T{*{AA|D|-}}}:{Uunknown/unwise URL type}{S>}}C"
#define URLS_SYNTAX \
        URL_SYNTAX "{*,W" URL_SYNTAX "}"
const static char url_syntax[] = URL_SYNTAX;
const static char urls_syntax[] = URLS_SYNTAX;
const static char MTA_SYNTAX[] = \
        "{Edns:;WDC|" \
...
const static char newslist_syntax[] = \
        NEWSGROUP_SYNTAX "{*," NEWSGROUP_SYNTAX "}W";
const static char type_syntax[] = \
...
#define HEAD_MTRKM 8
#define HEAD_MTRKR 9
const static char *headtypename[] = {
...
struct headmap {
     ...
     *  Auto-Submitted
     * RFC 6710
     *  priority clause in Received
     * RFC 6729
     *  state clause in Received
     * Non-standard:
    ...
    "auto-submitted",     mischead,
      "{A{E:auto-submitted value:no|auto-generated|auto-replied}|"
      "A{Wunrecognized value}}"
      "{*W;WA=S{Wunrecognized parameter}}C",
      TSPECIAL,
...
#define K_FORMAT   5
    ...
    "from",               fromhead, ALIST_SYNTAX,    STRUCTURED | HF_FROM,
    ...
    "list-subscribe",     mischead, urls_syntax,     TSPECIAL,
    "list-unsubscribe",   mischead, urls_syntax,     TSPECIAL,
    ...
    "return-receipt-to",  mischead,
      "LW@WDC{Wnon-standard header deprecated in favor of DSNs (" RFC_DSN ")}",
	                                             STRUCTURED,
    "sender",             mischead, SADDRGP_SYNTAX,  STRUCTURED | HF_SENDER,
...
/* table for specials
 */
const static unsigned char specials[] = "()<>@,;:\\\".[]";
const static unsigned char tspecials[] = "()<>@,;:\\\"/[]?=";
const static unsigned char bspecials[] = "'()+_,-./:=? 0123456789";
 ...
 */
#define XX 127
const static char decode64_table[256] = {
...
int web_cgi = 0;
int do_sendmail = 0;
const static char *reply_line, *from_addr = NULL;
static int reply_len;

/* strings for whitespace warnings
 */
const static char whitespace_str[] = "whitespace";
const static char comment_str[] = "comment";

/* message prefixes
 */
const static char w_none[]    = "";
const static char w_fatal[]   = "FATAL: ";
const static char w_error[]   = "ERROR: ";
const static char w_warn[]    = "WARNING: ";
const static char w_unknown[] = "UNKNOWN: ";
const static char w_ok[]      = "OK: ";
const static char pad_str[]   = "                    ";
...
static int tokcomp(const char *instr, int len, msgtoken *tok)
{
	...
	if (*str != TOLOWER(*tokstr)) return ((int) *str - *tokstr);
	++str, ++tokstr, --len, --tlen;
    }
			     ...
			     const char *dat, int len, int toknum)
{
    ...
    const static char uninit[] = "uninitialized token";
 ...
 */
const static char *skipvalgroup(const char *scan)
{
    int level = 0;
 ...
 */
const static char *skipvalseg(const char *scan)
{
    int level = 0;
...
{
    if (wht->last != NULL) {
	wprintf(db, w_warn, "%s after %T is obsolete", wht->lasttype,
	       wht->last);
    }
    warnupdate(wht, db);
			  ...
			  int *olen, const char **pfailstr, dbinfo *db)
{
    ...
    *olen = -1;
	...
	switch (*tmpl) {
	  ...
	  case 'T':
	    if ((special_table[(unsigned char) *data] & TSPECIAL_BIT) != 0) {
		if (vcreset(&stack, vs, &data, &tmpl)) continue;
		*pfailstr = "a token char (" RFC_MIME_TOKEN ")";
		goto DONE;
	    }
	    break;
 ...
 *  I       optionally signed integer
 *  i       optionally negated number 
		...
		if (tmpl[1] != 'C' ||
			...
			switch (atoi(tmpl + 2)) {
			  ...
			  case 1:
			    ...
			    break;
			  case 2:
			    failstr = "obsolete syntax: empty element in comma seprated list";
			    break;
			  case 3:
	...
	switch (*tmpl) {
	  ...
	  case 'I':
		...
		if (*tmpl == 'I')
		    ...
		    wprintf(db, w_error, "%T should be an optionally negated number", cur);
	    }
	    cur = cur->next;
	    break;
	  ...
	  RETRY_LOCAL_PART:
		...
		for (j = 0; j < cur->len && cur->data[j] > ' ' &&
		     cur->data[j] < 127 &&
		     (cur->data[j] == '.' ||
		      (special_table[cur->data[j]] & SPECIAL_BIT) == 0); ++j)
		    ;
		if (j == cur->len) {
	  ...
	  case '{':
		...
		while (*tmpl != '\0' && *tmpl != '}' && *tmpl != ':') {
			...
			((*tmpl == 'M' || *tmpl == 'O' || *tmpl == 'S') &&
			 (j < 1 || j > sizeof (unsigned long) * 8))) {
			...
			break;
		      ...
		      case 'S': /* in sequence up to this point */
			cseq = j;
			break;
		    }
		    while (*tmpl != ':' && *tmpl != '|' &&
		...
		do {
			...
			if (cond_enum && j > 0 && j < 33) {
			    if ((cseen & (1UL << (j - 1))) != 0)
			        wprintf(db, w_error, "duplicate %.*s '%t'",
				        llen, label, cur);
			    else
			    {
			        cseen |= (1UL << (j - 1));
				if (j > chi)
				    chi = j;
			            ...
			            wprintf(db, w_error, "out of order %.*s '%t'",
				            llen, label, cur);
			    }
			}
			tmpl = tokend;
		...
		tokend = skipvalgroup(tmpl) - 1;
		++tmpl;
		validatechars(&tmpl, cur->data, cur->len, &j, &failstr, db);
		if (j == -1) {
...
{
    const static char iso8859[] = "iso-8859-";
			...
			int *plen, int htype)
{
    ...
    *pmsg = line;
    ...
    switch (htype) {
		...
		wprintf(NULL, w_ok, "None of To, CC, or BCC "
			"fields specified in lines %d-%d - " RFC_TOCCBCC
			" used to require at least one such field",
			...
			int preamble)
{
    ...
    int sawhictl, saw8bit, sawsoft, softspace, qplen, endline, sawesc;
    utf8state ut8;
    int badqp, r;
    cteid cte;
    MIMEinfo *minfo;
	...
	if (cte == quoted_printable) {
	    ...
	    dat = line;
	    dst = databuf;
	    while (dat < msg && dst - databuf < sizeof (databuf)) {
...
{
    const static char pgpsig[] = "pgp-signature";
    const static char appl[] = "application/";
...
int msglint_parsemessage(const char *msg, int len)
{
	...
	wprintf(NULL, "Content-Type: ",
		"multipart/mixed; boundary=\"%x=_%x\"",
		bound, (unsigned int)pid);
	if (saw8bit) {
	    fprintf(outfile, "Content-Transfer-Encoding: 8bit\n");
	}
	fprintf(outfile, "Message-ID: <mimelint.%x.%x@%s>\n",
                bound, (unsigned int)pid, hostname);
	fprintf(outfile, "\n");
	fprintf(outfile, "--%x=_%x\n", bound, (unsigned int)pid);
	fprintf(outfile, "\n");
    }
...
void msglint_setup(int verbose_, int quiet_, int silent_, int mime_msg_,
                   int show_md5_, int web_cgi_, int do_sendmail_,
                   char *from_addr_, FILE *outfile_)
{
    verbose = verbose_;
    quiet = quiet_;
    silent = silent_;
    ...
    show_md5 = show_md5_;
    web_cgi = web_cgi_;
    do_sendmail = do_sendmail_;
    from_addr = from_addr_;
    outfile = ((outfile_ != NULL) ? outfile_ : stdout);
}
...
int main(int argc, char **argv)
{
	...
	r = msglint_parsemessage(buf, used);
